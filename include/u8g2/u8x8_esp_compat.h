...
#define U8X8_ESP_PIN_NONE 255
#define U8X8_ESP_PIN_CNT (U8X8_PIN_OUTPUT_CNT + U8X8_PIN_INPUT_CNT)
 ...
 */
uint8_t u8x8_esp_gpio_and_delay_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) ;

/**
 * Class for interfacing the MAX7219 8x8 LED matrix display using ESP32
 * with 4-wire software SPI.
 */
class U8X8_ESP_MAX7219_8X8_4W_SW_SPI : public U8X8 {
    public: std::array<uint8_t, U8X8_ESP_PIN_CNT> pins{};

    U8X8_ESP_MAX7219_8X8_4W_SW_SPI(uint8_t clock, uint8_t data, uint8_t cs, uint8_t reset) : U8X8() {
        u8x8_t* u8x8 = getU8x8();

        // set pin roles
        pins.fill(U8X8_ESP_PIN_NONE);
